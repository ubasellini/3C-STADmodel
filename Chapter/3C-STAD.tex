...
Outcomes from the SSE model allow us to obtain (i) the age-at-death distribution of each component over time (via Equation \eqref{Eq:FxDecom}), and (ii) the expected number of deaths separated by component, $\hat{\bm{d}}_k=\bm{e}\,\hat{\bm{\gamma}}_k$. This allows us to model and forecast age-at-death distributions independently for each component. 
...
The top panels in Figure \ref{Fig:STADexample} explain graphically the mechanisms underlying the STAD model for the Senescence component. Given a standard distribution (black lines in the graphs), let us consider the simpler case in which we vary the parameter $\nu_{s}$ but keep the compression parameters equal to 1, that is, $b_{s}^{\ell} = b_{s}^{u} = 1$. The transformation function in Equation (\ref{Eq:STADsen}) then simplifies to $t_{s}(x) = x - \nu_{s}$, and the resulting distribution is shifted along the $x$-axis by an amount equal to $\nu_{s}$. This case corresponds to a shifting mortality scenario (blue lines in the graphs): the new distribution has the same shape and variability of the standard, but it is  translated by the shifting parameter.

A more general development of mortality can be described by different values of the compression parameters, which act jointly with $\nu_{s}$ to modify the age-pattern of the standard distribution. When the two parameters are greater (lower) than 1, the variability of the segmented distribution is reduced (expanded) before and after the modal age at death with respect to the standard. In the top right panel of Figure \ref{Fig:STADexample}, the segmented distribution has a lower variability ($b_{s}^{\ell} > 1$) before the mode and a higher variability ($b_{s}^{u} < 1$) above the mode as compared to the standard distribution. As such, the two parameters capture the compression dynamic of mortality, distinguishing between changes that occur before and after the modal age at death.
...
Being equipped with the component-specific transformation functions, we can move from the theoretical description of the 3C-STAD model to its actual application for modelling and forecasting a series of age-at-death distributions over time. The first step needed to achieve this goal is the choice of the standard distribution $f_{k}(x)$ for each component. For the Senescent component, we select the standard as the mean of the aligned distributions, using a landmark registration approach \citep{ramsay2005FDA}. The alignment of the observed distributions indeed allows not to conflate features of the distributions that occur at different distances with respect to the mode \cite[see][pp.~4--6]{basellini2019stad}. For the Childhood and Early-adulthood components, we choose the standard as simple means of the observed distributions, as the alignment procedure is not required for the former, and it does not significantly improve the fit for the latter. 

\begin{table}[t]
	\centering
	\begin{tabular}{l|l|c|c|l}
		\multicolumn{1}{c|}{}                                & \multicolumn{1}{c|}{}                                                 & \multicolumn{2}{c|}{Parameters}         & \multicolumn{1}{c}{}                                  \\
		\multicolumn{1}{l|}{\multirow{-2}{*}{Component, $k$}} & \multicolumn{1}{l|}{\multirow{-2}{*}{Transformation, $t_{k}(\cdot)$}} & Shift     & Compression& \multicolumn{1}{l}{\multirow{-2}{*}{Standard, $f_{k}(x)$}} \\ \midrule 
		\rowcolor{my-grey}  
...
\end{equation}
%
where $\hat{\mu}^{k}_{x,t}$ denotes the hazard of component $k$ corresponding to the transformed distribution derived from $t_{k}(\cdot)$ applied in year $t$ to the associated standard $f_{k}(x)$. Note that the vector $\bm{\theta}_{k,t}$ contains only the compression parameter(s). For each year $t$, the shifting parameters $\nu_{s}$ and $\nu_{e}$ of the Senescent and Early-adulthood components are computed as differences in the modal age at death between standard and observed distributions, as estimated by the SSE model. 
...
\begin{landscape}
	...
	\centering
	\scriptsize
	\begin{tabular}{lC{1.3cm}C{1.3cm}cC{1cm}|C{1cm}|C{1cm}|C{1cm}|C{1cm}|C{1cm}|C{1cm}|C{1cm}}
	\toprule & & & & \multicolumn{8}{c}{\textbf{Forecast Accuracy}} \\ \cmidrule{5-12} 
	& & &  &  \multicolumn{4}{c}{\textbf{Point forecast }(MAE)}    & \multicolumn{4}{c}{\textbf{Interval forecast} (ECP for 80\% PI)} \\ 
	\cmidrule{5-12} Country $\quad$ & Training period & Validation period &   Measure  &  3C-STAD & LC & CODA & HU & 3C-STAD & LC & CODA & HU  \\
	\midrule
	...
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} &  \multicolumn{1}{c|}{\cellcolor{my-grey}}  & \multicolumn{1}{c|}{\cellcolor{my-grey}$e^{0}$} & \textbf{0.13}  & 0.34   & 0.37  & 0.15   & 100\% & 100\% & 100\% & \textbf{90\%} \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$g^{0}$} &\textbf{0.07} & 0.13   & 0.09 & \textbf{0.07}   & 100\% &  70\%  & 100\% & \textbf{80\%}    \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}1950-2006}} &  \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}10y}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$\ln(m_{x,t})$}      & \textbf{0.19} & 0.24  & 0.24  & 0.22   & 39\% & 32\% & \textbf{50\%} & 36\% \\ 
	\hhline{~|-----------|}
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}$e^{0}$} & 0.13  & 0.13   & 0.36  &  \textbf{0.12}   & 100\% & 100\% & 100\% & \textbf{95\%} \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}$g^{0}$} & 0.08  & \textbf{0.07} & 0.21  & 0.11 & 90\% & 100\% & 95\% & \textbf{85\%}  \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}{1950-1996}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}{20y}}} & \multicolumn{1}{c|}{\cellcolor{my-white}$\ln(m_{x,t})$}      & \textbf{0.19} & 0.24  & 0.23 & 0.23   & 48\% & 51\% & \textbf{52\%} & 38\%  \\ 
	\hhline{~|-----------|}
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$e^{0}$} & 0.82 & 0.53  & 0.48  & \textbf{0.16}   & \textbf{77\%} & 100\% & 87\% & 93\%     \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$g^{0}$} & 0.21 & 0.43 & 0.32 & \textbf{0.13} & 70\%   & 63\% &  27\%  & \textbf{80\%}   \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\multirow{-9}{*}{\cellcolor{my-grey}\thead{Switzerland \\ Females}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}1950-1986}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}30y}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$\ln(m_{x,t})$}      & 0.22 & 0.29   & 0.23 & \textbf{0.20} & 51\% & 49\% & \textbf{59\%}  & 41\%  \\ 	\midrule
	
	%% CHE Males
	
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &     \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}$e^{0}$} & \textbf{0.16} & 0.37  & 0.29  & 0.77   & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} & 0\%   \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &     \multicolumn{1}{c|}{\cellcolor{my-white}}                 & \multicolumn{1}{c|}{\cellcolor{my-white}}    & \multicolumn{1}{c|}{\cellcolor{my-white}$g^{0}$} & 0.12  & 0.20  & \textbf{0.11}  & 0.26   & 50\% & \textbf{70\%} & 100\% & 10\%  \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}1950-2006}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}10y}} & \multicolumn{1}{c|}{\cellcolor{my-white}$\ln(m_{x,t})$} & \textbf{0.17}   & 0.23  & 0.20 &  0.20   & 34\% & 31\% & \textbf{53\%} & 31\%  \\ 
	\hhline{~|-----------|}
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &         \multicolumn{1}{c|}{\cellcolor{my-grey}}             & \multicolumn{1}{c|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-grey}$e^{0}$} & \textbf{1.40} & 1.61  & 1.57 & 2.23  & 10\% & \textbf{25\%} & 20\%  &  0\%    \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &      \multicolumn{1}{c|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-grey}}   & \multicolumn{1}{c|}{\cellcolor{my-grey}$g^{0}$} & \textbf{0.45} & 0.74  & 0.62 & 1.01   & \textbf{15\%} & 5\%  & \textbf{15\%} & 0\%  \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}{1950-1996}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}{20y}}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$\ln(m_{x,t})$} & \textbf{0.25} & 0.29  & 0.28 & 0.32   & 28\% & 33\% & \textbf{42\%}  & 17\% \\ 
	\hhline{~|-----------|}
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &   \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}$e^{0}$} & 1.60 & \textbf{1.58}  & 2.14  & 1.93  & 33\% & \textbf{43\%} & 33\% & 17\% \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &  \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}$g^{0}$} & 0.98 & \textbf{0.62} &  0.86  & 0.70  & 17\% & \textbf{23\%} & \textbf{23\%} & 20\%  \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\multirow{-9}{*}{\cellcolor{my-white}\thead{Switzerland \\ Males}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}1950-1986}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}30y}} & \multicolumn{1}{c|}{\cellcolor{my-white}$\ln(m_{x,t})$} & 0.32 & \textbf{0.27} & 0.30 & \textbf{0.27} & 22\% & 38\% & \textbf{44\%} & 21\%  \\ \midrule
	
	%% SWE Females %% SWE Females %% SWE Females
	%% SWE Females %% SWE Females %% SWE Females
	
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} &  \multicolumn{1}{c|}{\cellcolor{my-grey}}  & \multicolumn{1}{c|}{\cellcolor{my-grey}$e^{0}$} & \textbf{0.12}   & \textbf{0.12}    & 0.39   & 0.16  & 100\% & 100\% & 100\% & \textbf{90\%} \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$g^{0}$} & 0.07   & \textbf{0.05}    & 0.12  & 0.12   & \textbf{80\%} & 90\% & 100\% & 50\%    \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}1950-2006}} &  \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}10y}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$\ln(m_{x,t})$} & \textbf{0.16}  & 0.18   & 0.22   & \textbf{0.16}   & 37\% & 37\%  & \textbf{53\%} & 42\% \\ 
	\hhline{~|-----------|}
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}$e^{0}$} & 0.45   &  \textbf{0.16}    & 0.69  & 0.44 & 100\% & 100\% & \textbf{90\%} & 20\% \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}$g^{0}$} & \textbf{0.07}  & 0.10    & 0.19   & \textbf{0.07} & 100\% & 90\% & 95\% & \textbf{75\%} \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}{1950-1996}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}{20y}}} & \multicolumn{1}{c|}{\cellcolor{my-white}$\ln(m_{x,t})$} & \textbf{0.17} & 0.19  & 0.21 & 0.19 & 49\% & 46\% & \textbf{54\%} & 26\% \\ 
	\hhline{~|-----------|}
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$e^{0}$} & 0.69    & 0.49    & 1.56     & \textbf{0.46}   & 27\% & \textbf{97\%} & 7\% & 30\%     \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-grey}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$g^{0}$} & 0.09  &  \textbf{0.08}    & 0.40     & \textbf{0.08}   & \textbf{83\%} & 87\% & 13\% & 67\%   \\
	\rowcolor{my-grey} 
	\multicolumn{1}{c|}{\multirow{-9}{*}{\cellcolor{my-grey}\thead{Sweden \\ Females}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}1950-1986}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}30y}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$\ln(m_{x,t})$} & \textbf{0.18} & 0.21   & 0.28   & 0.19 & 38\% & \textbf{39\%} & 31\% & 28\% \\ \midrule
	...
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &     \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}$e^{0}$} & \textbf{0.10}  & 0.36   & 0.49    & 0.81  & 100\% & 90\% & \textbf{80\%} & 0\%   \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &     \multicolumn{1}{c|}{\cellcolor{my-white}}                 & \multicolumn{1}{c|}{\cellcolor{my-white}}    & \multicolumn{1}{c|}{\cellcolor{my-white}$g^{0}$} & 0.09   & 0.10    & \textbf{0.04}  & \textbf{0.04} & \textbf{80\%} & 90\% & 100\% & 100\%  \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}1950-2006}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}10y}} & \multicolumn{1}{c|}{\cellcolor{my-white}$\ln(m_{x,t})$} &  \textbf{0.17}   & 0.23    &  0.19  &  0.18 & 42\% & 23\% & \textbf{44\%} & 26\%  \\ 
	\hhline{~|-----------|}
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &  \multicolumn{1}{c|}{\cellcolor{my-grey}}  & \multicolumn{1}{c|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-grey}$e^{0}$} & \textbf{0.71}   & 1.38     & 1.30   & 1.76 & \textbf{100\%} & 20\% & 10\% & 0\%   \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &      \multicolumn{1}{c|}{\cellcolor{my-grey}}    & \multicolumn{1}{c|}{\cellcolor{my-grey}}   & \multicolumn{1}{c|}{\cellcolor{my-grey}$g^{0}$} & 0.35   & 0.38     & \textbf{0.25}    & 0.42 & 25\% & 10\% & \textbf{40\%} & 0\%  \\
	\rowcolor{my-grey} 
	\multicolumn{1}{l|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}{1950-1996}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-grey}{20y}}} & \multicolumn{1}{c|}{\cellcolor{my-grey}$\ln(m_{x,t})$} & \textbf{0.20} & 0.29  & 0.23 & 0.22 & \textbf{50\%} & 22\% & 33\% & 25\% \\ 
	\hhline{~|-----------|}
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &   \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}} & \multicolumn{1}{c|}{\cellcolor{my-white}$e^{0}$} & 2.74   & \textbf{2.46}   & 2.58    & 3.12   & 7\% & \textbf{23\%} & 7\% & 0\% \\
	\rowcolor{my-white} 
	\multicolumn{1}{l|}{\cellcolor{my-white}}    &  \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}}  & \multicolumn{1}{c|}{\cellcolor{my-white}$g^{0}$} & \textbf{0.44}    & 0.83    & 0.89    & 1.02   & 17\% & \textbf{20\%} & 10\% & 10\%  \\
	\rowcolor{my-white} 
	\multicolumn{1}{c|}{\multirow{-9}{*}{\cellcolor{my-white}\thead{Sweden \\ Males}}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}1950-1986}} & \multicolumn{1}{c|}{\multirow{-3}{*}{\cellcolor{my-white}30y}} & \multicolumn{1}{c|}{\cellcolor{my-white}$\ln(m_{x,t})$} & \textbf{0.30}  & 0.37  & 0.31  & 0.31 & 17\% & \textbf{31\%} & 25\% & 16\%  \\ 
	
	\bottomrule 
				
	\end{tabular}
	\caption{Mean absolute error (MAE) and empirical coverage probability (ECP) for the 80\% PI of the 3C-STAD, LC, CODA and HU forecasts of $e^{0}$, $g^{0}$ and $\ln(m_{x,t})$ for females and males in two countries and three out-of-sample exercises: validation periods of 10 years (training period 1950-2006), 20 years (1950-1996) and 30 years (1950-1986). Lower values of the MAE (in bold) correspond to greater point forecast accuracy. Values of the ECP closer to the 80\% nominal level (in bold) correspond to greater interval forecast accuracy.\\\footnotesize{\textit{Source}: as for Figure \ref{Fig:Intro}.}}\label{Table:Validation}
	\end{table}
